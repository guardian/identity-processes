...
object FormstackConfig {
  ...
  def getPerformSarHandlerConfig: PerformSarLambdaConfig =
    ...
    } yield PerformSarLambdaConfig(resultsBucket, resultsPath, encryptionPassword, FormstackAccountToken(1, accountOneToken), FormstackAccountToken(2, accountTwoToken), bcryptSalt))
      .getOrElse {
        throw new RuntimeException(
